...
class SE_Block(nn.Module):
    def __init__(self, inchannel, ratio=16):
        ...
        # Two fully connected layers (Fex operation)
    ...
    def forward(self, x):
        #  Read the number of images and the number of channels in the batch data.
        b, c, h, w = x.size()
        # Fsq operation: After pooling, the output is a matrix of size b*c.
        y = self.gap(x).view(b, c)
        # Fex operation: After passing through the fully connected layers, the output is a matrix of size (b, c, 1, 1).
        y = self.fc(y).view(b, c, 1, 1)
        # Fscale operation: Multiply the obtained weights by the original feature map x.
...
class ExamPle(nn.Module):
    def __init__(self):
        ...
        self.embedding_ss = nn.Embedding(4, self.emb_dim, padding_idx=0)
        self.encoder_layer_seq = nn.TransformerEncoderLayer(d_model=512, nhead=16, dropout=0.2)
        self.encoder_layer_ss = nn.TransformerEncoderLayer(d_model=512, nhead=16, dropout=0.2)

        # Pass the sequence information and secondary structure information through different transformer encoders
        self.transformer_encoder_seq = nn.TransformerEncoder(self.encoder_layer_seq, num_layers=2)
        self.transformer_encoder_ss = nn.TransformerEncoder(self.encoder_layer_ss, num_layers=1)

        self.gru_seq = nn.GRU(self.emb_dim, self.hidden_dim, num_layers=2,
                              bidirectional=True, dropout=0.3)
        self.gru_ss = nn.GRU(self.emb_dim, self.hidden_dim, num_layers=2,
                             bidirectional=True, dropout=0.3)
    ...
    def forward(self, x, ss):
        se_model = SE_Block(inchannel=512).cuda()
        x = self.embedding_seq(x)
        output = self.transformer_encoder_seq(x).permute(1, 0, 2)
        # trans->SEnet
        se_output = torch.zeros_like(output).cuda()
        for i in range(output.size(1)):
            channel = output[:, i, :].unsqueeze(1)
            channel = channel.view(-1, channel.size(2), 1, 1)
            channel_se = se_model(channel).cuda()
            se_output[:, i, :] = channel_se.view(-1, 512)
        output = output + se_output
        # SEnet->gru
        ...
        ss = self.embedding_ss(ss)
        ss_output = self.transformer_encoder_ss(ss).permute(1, 0, 2)

        ss_se_output = torch.zeros_like(ss_output).cuda()
        for i in range(ss_output.size(1)):
            ss_channel = ss_output[:, i, :].unsqueeze(1)
            ss_channel = ss_channel.view(-1, ss_channel.size(2), 1, 1)
            ss_channel_se = se_model(ss_channel).cuda()
            ss_se_output[:, i, :] = ss_channel_se.view(-1, 512)
        ss_output = ss_output + ss_se_output

        ss_output, ss_hn = self.gru_ss(ss_output)
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        loss_contrastive = torch.mean((1 - label) * torch.pow(euclidean_distance, 2) +
                                      label * torch.pow(torch.clamp(self.margin - euclidean_distance, min=0.1),
                                                        2))  # 修改0.0为0.1
